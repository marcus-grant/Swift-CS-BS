...
class SortController: NSObject {
  ...
  private var randomState = UInt32(-1*NSDate().timeIntervalSinceNow)
  ...
  func createTestArray(numElements: Int, minValue: Int, maxValue: Int) -> [Int] {
  ...
  func testCombSortWithInts(trials: Int, size: Int, gapScale: Double, minValue: Int, maxValue: Int)
    ...
    while currentTrial < trials {
      var testArray =
        createTestArray(size, minValue: minValue, maxValue: maxValue)
  ...
  //MARK: - Stats funcs
  func calculateStats(data: [Double])
